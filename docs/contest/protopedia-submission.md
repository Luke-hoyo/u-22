...
単なる画面デモではなく、**若者・事業者・自治体・運営の4者が同じデータ上で動く**ところまで再現しています。
...
```text
【設計の方針】
「はたるくん」は、若者・事業者・自治体・運営の4者が同じデータを扱う前提で設計しました。
...
【どう作ったか（構築手順）】
1. 要件整理
   求人マッチング、奨学金免除見込み、地域ポイント、事業者・運営の管理画面を必須機能として定義。
2. UI 設計
...
4. データ層
   Appwrite TablesDB に求人・応募・ユーザー・ポイント・イベント・事業者申請・通知を設計。
...
### 事業者・自治体・運営向け（Web ダッシュボード / Mobile 事業者画面）
...
| 1:40–2:20 | **事業者デモ**: 求人作成 → 応募確認 → ポイント付与 | 事業者ダッシュボード |
...
2. 【PC または Mobile 事業者画面】事業者アカウントでログイン
   → 求人作成・公開 → 応募者ステータス更新
...
1. 事業者アカウントで `/farmer/dashboard` にログイン
2. 新規求人を作成・公開
3. 応募者のステータスを更新
4. 運営アカウントで事業者申請・ポイント申請を承認
