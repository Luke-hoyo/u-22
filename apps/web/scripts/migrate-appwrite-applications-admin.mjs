...
async function ensureStringColumn(key, size, required, xdefault = "") {
  try {
    ...
    console.log(`Added applications column: ${key}`);
  } catch (error) {
    if (error?.code === 409) {
      console.log(`Applications column already exists: ${key}`);
      return;
    }
...
await ensureStringColumn("reviewStatus", 32, false, "new");

console.log("Applications admin column migration completed.");
