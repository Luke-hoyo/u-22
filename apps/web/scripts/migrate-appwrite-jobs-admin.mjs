...
const tableId = process.env.APPWRITE_TABLE_ID_JOBS.trim();
...
async function ensureStringColumn(key, size, required, xdefault = "") {
  try {
    ...
    console.log(`Added jobs column: ${key}`);
  } catch (error) {
    if (error?.code === 409) {
      console.log(`Jobs column already exists: ${key}`);
      return;
    }
...
async function ensureIntegerColumn(key, required, min, max, xdefault = 0) {
  try {
...
await ensureStringColumn("status", 32, false, "published");
await ensureIntegerColumn("capacity", false, 1, 30, 2);
await ensureStringColumn("ownerClerkUserId", 64, false, "");

console.log("Jobs admin column migration completed.");
